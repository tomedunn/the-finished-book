...
It's also worth noting that for a typical chance of a target failing their saving throw of $$\rho_{\fail} = 0.65$$, the average damage for $$m_{\save} = 0.5$$ is  $$\mu_{\action} = 0.82 \, \mu_{\fail}$$, which is roughly $$25\%$$ higher than it is for $$m_{\save} = 0.0$$, which has an average damage of $$\mu_{\action} = 0.65 \, \mu_{\fail}$$. This explains why the rules for [creating a spell](https://www.dndbeyond.com/sources/dmg/dungeon-masters-workshop#SpellDamage) from chapter 9 of the _Dungeon Master's Guide_ recommend increasing the damage for spells with $$m_{\save} = 0.0$$ by $$25\%$$ over the values listed in that section.
