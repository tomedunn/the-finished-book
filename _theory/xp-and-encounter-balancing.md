...
\begin{align}
    \XPtot_\mathrm{PCs}\left(\diff\,\right) = \sum_i \XP\_{\,\PC}\left(L\_{i}, \diff\,\right)\,, \label{eq:xp-threshold-total-dmg}
\end{align}

where $$\diff$$ represents the difficulty threshold being targeted, and $$L$$ represents a PC's level.

The next step in the process is to calculate the total XP for NPCs in the encounter, $$\XPtot_\mathrm{NPCs}$$, by adding up the individual XP values for each of the enemy NPCs,

\begin{align}
    \XPtot_\mathrm{NPCs} = \sum_i \XP\_{\,\NPC}\left(\CR\_{i}\,\right)\,, \label{eq:xp-total-dmg}
\end{align}
...
<figure id="fig:xp-encounter-diagram">
    ...
    <figcaption>Figure 3: Graphical representation of the RHS of Eqn. \eqref{eq:difficulty-xp} for and encounter with three NPCs.</figcaption>
</figure>
